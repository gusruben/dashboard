<script lang="ts">
    ...
    $: cellSize = containerHeight > 0 ? Math.min((containerHeight - 32) / 7, (containerWidth - 32) / weeksToShow) : 10;
    ...
    $: commitData = (() => {
        ...
        const daysBetween = Math.ceil((endDate.getTime() - displayStartDate.getTime()) / (1000 * 60 * 60 * 24));
...
<div bind:this={containerElement} class="bg-gray-900 rounded-3xl flex-1/3 p-4 h-full overflow-hidden">
            ...
            {#snippet children({ context })}
                ...
                <Tooltip.Root>
                    {#snippet children({ data })}
                        <div class="bg-gray-700 text-white px-2 py-1 rounded text-xs">
